...
static void session_state_cb(rtc_session_state_t state, void *ctx);
...
static void apply_state(ui_state_t state)
{
    ...
    case UI_STATE_HOST:
        lv_label_set_text(s_title_label, "Host Mode - RTC AI");
...
// ---------------------------------------------------------------------------
// Background helpers — rtc ops must NOT run in the LVGL callback context
// ---------------------------------------------------------------------------
...
//  • Blocking work (I2C, network, rtc) dispatched to background task.
//  • Meeting mode (HTTP/WS) is MOCKED — only Host Mode uses real RTC.
// ---------------------------------------------------------------------------
...
static void btn_host_cb(lv_event_t *e)
{
    ESP_LOGI(TAG, "[btn] Host Mode — starting RTC session");
    apply_state(UI_STATE_HOST);
    lv_label_set_text(s_status_label, "* Connecting...");
    // Real RTC session — dispatched to background task
...
static void btn_exit_cb(lv_event_t *e)
{
    ESP_LOGI(TAG, "[btn] Exit Host Mode — stopping RTC session");
...
// ---------------------------------------------------------------------------
// Session state callback — fired from rtc_session internal task
// ---------------------------------------------------------------------------
static void session_state_cb(rtc_session_state_t state, void *ctx)
{
    const char *text;
